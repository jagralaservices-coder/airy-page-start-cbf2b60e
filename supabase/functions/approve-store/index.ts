...
serve(async (req) => {
  ...
  try {
    ...
    if (!roleData) return new Response(JSON.stringify({ error: 'Only admins can approve stores' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    ...
    await supabaseAdmin.from('audit_logs').insert({
      actor_id: user.id, actor_email: user.email, action: 'approve_store',
      entity_type: 'store', entity_id: store_id, before_data: before,
      after_data: { approval_status: 'approved', is_active: true },
    })
